...
import { deleteOrder, getAllBulksOrders, getAllSampleOrders } from '@/app/action/orders-action';
...
const BulksCards = () => {
    ...
    const { data: allSampleOrders, error, isLoading, refetch } = useQuery({
        queryKey: ['allBulksOrders'],
        queryFn: getAllBulksOrders, // No user ID required here
    });
                        ...
                        {allSampleOrders.slice(0, visibleCount).map((order, i) => (
                            ...
                            >
                                ...
                                <div className="flex justify-between items-center">
                                    ...
                                    <Link href={`/dashboard/bulks/detail?id=${order.id}`} className="bg-white hover:bg-black hover:text-white text-black px-6 py-3 rounded-full">
                                        Expand
                                    </Link>
                                </div>
                            </div>
